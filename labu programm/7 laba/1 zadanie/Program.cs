...
namespace _1_zadanie
{
    class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            string directory = @"E:\Program po 3d modelirovanii-o\Rabotu visual studio\labu programm\7 laba\1 zadanie";
            ...
            using (StreamReader readFile = new StreamReader(path))
            {
                ...
                {
                    string marhrut = readFile.ReadLine();
                    string pos = readFile.ReadLine();
                    var vidTransporta = Vid.Тр;
                    if (pos == "Тр")
                    {
                    ...
                    decimal protiagennost = decimal.Parse(readFile.ReadLine());
                    int vremia = int.Parse(readFile.ReadLine());
                    Worker newWorker;
            ...
            do
            {
                ...
                Console.WriteLine("7 - Сортировка по времени.");
                ...
                if (sektor == 7)
                {
                    for (int i = 0; i < tabl.Count; i++)
                    {
                        int j = i - 1;
                        var temp = tabl[i];
                        while (j >= 0 && temp.vremia < tabl[j].vremia)
                        {
                            tabl[j + 1] = tabl[j];
                            j--;
                        }
                ...
                if (sektor == 8)
                {
                    ...
                    {
                        for (int i = 0, k = 1; i < tabl.Count; i++)
                        {
                            newText.WriteLine(k + "){0}\n{1}\n{2}\n{3}\n", tabl[i].vidTransporta, tabl[i].marhrut, tabl[i].protiagennost, tabl[i].vremia);
                            k++;
                        }
                    }
                    working = false;
                }
                if (sektor < 1 || sektor > 8)
                {
                    Console.WriteLine("Выберите действие от 1 до 8!");
                    Console.WriteLine();
                }
            }
            while (working);
